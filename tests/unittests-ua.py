...
def read(key, response, test_type='original-conll', matching="exact", allow_boundary_crossing=False, keep_zeros=False):
  args = {
    "format": 'ua',
  ...
  test_dir = os.path.join('tests-ua', test_type)
  reader.get_coref_infos(os.path.join(test_dir, key), os.path.join(test_dir, response), unit_test=True)
...
def test_O2_partial():
  doc = read('TC-O.key', 'TC-O-2.response', 'discontinuous-craft', matching='partial-craft')
  assert evaluate(doc, muc) == approx([1,1,1])
  # assert evaluate(doc, b_cubed) == approx([6/6, 6/6, 1],abs=TOL)
  # assert evaluate(doc, ceafe) == approx([1,1,1],abs=TOL)
  # assert evaluate(doc, ceafm) == approx([1,1,1],abs=TOL)
...
def test_O3_partial():
  doc = read('TC-O.key', 'TC-O-3.response', 'discontinuous-craft', matching='partial-craft')
  assert evaluate(doc, muc) == approx([2/3,2/3,4/6])
  # assert evaluate(doc, b_cubed) == approx([6/6, 6/6, 1],abs=TOL)
  # assert evaluate(doc, ceafe) == approx([1,1,1],abs=TOL)
  # assert evaluate(doc, ceafm) == approx([1,1,1],abs=TOL)
...
def test_P2_partial():
  doc = read('TC-P.key', 'TC-P-2.response', 'discontinuous-craft', matching='partial-craft')
  assert evaluate(doc, muc) == approx([2/3,2/3,4/6])
  # assert evaluate(doc, b_cubed) == approx([6/6, 6/6, 1],abs=TOL)
  # assert evaluate(doc, ceafe) == approx([1,1,1],abs=TOL)
  # assert evaluate(doc, ceafm) == approx([1,1,1],abs=TOL)
...
def test_P3_partial():
  doc = read('TC-P.key', 'TC-P-3.response', 'discontinuous-craft', matching='partial-craft')
  assert evaluate(doc, muc) == approx([1,1,1])
  # assert evaluate(doc, b_cubed) == approx([6/6, 6/6, 1],abs=TOL)
  # assert evaluate(doc, ceafe) == approx([1,1,1],abs=TOL)
  # assert evaluate(doc, ceafm) == approx([1,1,1],abs=TOL)
...
def test_Q2_partial():
  doc = read('TC-Q.key', 'TC-Q-2.response', 'discontinuous-craft', matching='partial-craft')
  assert evaluate(doc, muc) == approx([1, 1, 1])
  # assert evaluate(doc, b_cubed) == approx([6/6, 6/6, 1],abs=TOL)
  # assert evaluate(doc, ceafe) == approx([1,1,1],abs=TOL)
  # assert evaluate(doc, ceafm) == approx([1,1,1],abs=TOL)
...
def test_NCMA1():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-1.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA2():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-2.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA3():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-3.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA4():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-4.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA5():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-5.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA6():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-6.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA7():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-7.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB1():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-1.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB2():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-2.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB3():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-3.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB4():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-4.response',"partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB5():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-5.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB6():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-6.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB7():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-7.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB8():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-8.response', "partial-match-corefud", matching='partial-corefud')
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)
...
def test_OLMA1():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-1.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMA2():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-2.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMA3(): #UA support cross mention like (a (b a) b) but need using allow_boundary_crossing=true
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-3.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMA4():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-4.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)

def test_OLMA5():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-5.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (3/4, 3/3, 6/7)
  assert evaluate(doc, b_cubed) == (17/24, 5/5, 34/41)

def test_OLMA6():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-6.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)

def test_OLMB1():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-1.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMB2():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-2.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMB3(): #UA support cross mention like (a (b a) b) but need using allow_boundary_crossing=true
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-3.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
...
def test_OLMB4():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-4.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)

def test_OLMB5():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-5.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (3/4, 3/3, 6/7)
  assert evaluate(doc, b_cubed) == (17/24, 5/5, 34/41)

def test_OLMB6():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-6.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)

def test_OLMC1():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-1.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMC2(): #the test case seems not correct
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-2.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
...
def test_OLMC3():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-3.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == approx([3/4, 1, 6/7], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([17/24, 1, 34/41], abs=TOL)

def test_OLMC4():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-4.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
...
def test_OLMC5(): #we make sure mentions are one-to-one aligned so the calculation is different
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-5.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
  assert evaluate(doc, muc) == approx([1, 1, 1], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([1, 1, 1], abs=TOL)
...
@pytest.mark.skip(reason="errors in the test")
def test_OLMC6():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-6.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
...
@pytest.mark.skip(reason="errors in the test")
def test_OLMC7():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-7.response', "partial-match-corefud", matching='partial-corefud',allow_boundary_crossing=True)
