...
def read(key, response, matching="partial-corefud"):
  args = {
    "format": 'corefud',
  ...
  test_dir = os.path.join("tests-corefud", f"{matching}-match")
  reader.get_coref_infos(os.path.join(test_dir, key), os.path.join(test_dir, response))
  return reader.doc_coref_infos

def test_A1():
  doc = read('TC-A.key', 'TC-A-1.response', matching="exact")
...
def test_A2():
  doc = read('TC-A.key', 'TC-A-2.response', matching="exact")
...
def test_A3():
  doc = read('TC-A.key', 'TC-A-3.response', matching="exact")
...
def test_A4():
  doc = read('TC-A.key', 'TC-A-4.response', matching="exact")
...
def test_A5():
  doc = read('TC-A.key', 'TC-A-5.response', matching="exact")
...
def test_A6():
  doc = read('TC-A.key', 'TC-A-6.response', matching="exact")
  assert evaluate(doc, muc) == approx([1 / 3, 1 / 4, 2 / 7])
  assert evaluate(doc, b_cubed) == approx([
      (10 / 3) / 6, (1 + 4 / 3 + 1 / 2) / 8,
...
def test_A7():
  doc = read('TC-A.key', 'TC-A-7.response', matching="exact")
  assert evaluate(doc, muc) == approx([1 / 3, 1 / 3, 1 / 3])
  assert evaluate(doc, b_cubed) == approx([
      (10 / 3) / 6, (1 + 4 / 3 + 1 / 2) / 7,
      2 * (5 / 9) * (17 / 42) / ((5 / 9) + (17 / 42))
...
def test_A10():
  doc = read('TC-A.key', 'TC-A-10.response', matching="exact")
...
def test_A11():
  doc = read('TC-A.key', 'TC-A-11.response', matching="exact")
...
def test_A12():
  doc = read('TC-A.key', 'TC-A-12.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
  assert evaluate(doc, b_cubed) == approx([
      (1 + 1 / 2 + 2 / 3) / 6, 4 / 7,
...
def test_A13():
  doc = read('TC-A.key', 'TC-A-13.response', matching="exact")
...
def test_B1():
  doc = read('TC-B.key', 'TC-B-1.response', matching="exact")
...
def test_C1():
  doc = read('TC-C.key', 'TC-C-1.response', matching="exact")
...
def test_D1():
  doc = read('TC-D.key', 'TC-D-1.response', matching="exact")
...
def test_E1():
  doc = read('TC-E.key', 'TC-E-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
      [9 / 9, 9 / 10, 2 * (9 / 9) * (9 / 10) / (9 / 9 + 9 / 10)])
  assert evaluate(doc, b_cubed) == approx(
...
def test_F1():
  doc = read('TC-F.key', 'TC-F-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
...
def test_G1():
  doc = read('TC-G.key', 'TC-G-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
...
def test_H1():
  doc = read('TC-H.key', 'TC-H-1.response', matching="exact")
  assert evaluate(doc, muc) == approx([1, 1, 1])
  assert evaluate(doc, lea) == approx([1, 1, 1])


def test_I1():
  doc = read('TC-I.key', 'TC-I-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
      [2 / 3, 2 / 2, 2 * (2 / 3) * (2 / 2) / (2 / 3 + 2 / 2)])
  assert evaluate(doc, lea) == approx(
...
def test_J1():
  doc = read('TC-J.key', 'TC-J-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
...
def test_K1():
  doc = read('TC-K.key', 'TC-K-1.response', matching="exact")
...
def test_L1():
  doc = read('TC-L.key', 'TC-L-1.response', matching="exact")
  assert evaluate(doc, muc) == approx(
...
def test_M1():
  doc = read('TC-M.key', 'TC-M-1.response', matching="exact")
  assert evaluate(doc, muc) == approx([1, 1, 1])
...
def test_M2():
  doc = read('TC-M.key', 'TC-M-2.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
  assert evaluate(doc, lea) == approx([0, 0, 0])
  assert evaluate(doc, [blancc, blancn]) == approx([0, 0, 0])

def test_M3():
  doc = read('TC-M.key', 'TC-M-3.response', matching="exact")
  assert evaluate(doc, lea) == approx([
      6 * (4 / 15) / 6, (2 + 3 + 0) / 6,
      2 * 4 / 15 * 5 / 6 / (4 / 15 + 5 / 6)
  ])
...
def test_M4():
  doc = read('TC-M.key', 'TC-M-4.response', matching="exact")
  assert evaluate(doc, lea) == approx([
      6 * (3 / 15) / 6, 6 * (3 / 15) / 6,
...
def test_M5():
  doc = read('TC-M.key', 'TC-M-5.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
  assert evaluate(doc, lea) == approx([0, 0, 0])
  assert evaluate(doc, [blancc, blancn]) == approx([0, 0, 0])
...
def test_M6():
  doc = read('TC-M.key', 'TC-M-6.response', matching="exact")
  assert evaluate(doc, lea) == approx([
...
def test_N1():
  doc = read('TC-N.key', 'TC-N-1.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
  assert evaluate(doc, lea) == approx([1, 1, 1])
  assert evaluate(doc, [blancc, blancn]) == approx([1, 1, 1])
...
def test_N2():
  doc = read('TC-N.key', 'TC-N-2.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
  assert evaluate(doc, lea) == approx([0, 0, 0])
  assert evaluate(doc, [blancc, blancn]) == approx([0, 0, 0])
...
def test_N3():
  doc = read('TC-N.key', 'TC-N-3.response', matching="exact")
...
def test_N4():
  doc = read('TC-N.key', 'TC-N-4.response', matching="exact")
  assert evaluate(doc, muc) == approx([0, 0, 0])
...
def test_N5():
  doc = read('TC-N.key', 'TC-N-5.response', matching="exact")
  assert evaluate(doc, lea) == approx([0, 0, 0])
  assert evaluate(doc, [blancc, blancn]) == approx([0, 0, 0])


def test_N6():
  doc = read('TC-N.key', 'TC-N-6.response', matching="exact")
  assert evaluate(doc, lea) == approx([0, 0, 0])
  # the original is wrong as the |C_r| != 0
...
def test_O1():
  doc = read('TC-O.key', 'TC-O-1.response', matching="exact")
  assert evaluate(doc, b_cubed) == approx([4/9, 4/15, 1/3], abs=TOL)

############################# TESTS ON NON-CONTIGUOUS MENTIONS ##############################

def test_NCMA1():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-1.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA2():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-2.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA3():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-3.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA4():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-4.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA5():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-5.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA6():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-6.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA7():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-7.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB1():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-1.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB2():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-2.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB3():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-3.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB4():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-4.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB5():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-5.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB6():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-6.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB7():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-7.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB8():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-8.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)
...
def test_OLMA1():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-1.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMA2():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-2.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1 + 9/23))

def test_OLMA3():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-3.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (18/23, 1, 2*18/23/(1+18/23))

def test_OLMA4():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-4.response', matching="partial-corefud")
...
def test_OLMA5():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-5.response', matching="partial-corefud")
...
def test_OLMA6():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-6.response', matching="partial-corefud")
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMAA1():
  doc = read('overlapping_mentions/TC-OLMAA.key', 'overlapping_mentions/TC-OLMAA-1.response', matching="partial-corefud")
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMAA2():
  doc = read('overlapping_mentions/TC-OLMAA.key', 'overlapping_mentions/TC-OLMAA-2.response', matching="partial-corefud")
  assert evaluate(doc, mention_overlap) == approx([9/17, 9/10, 2*9/17*9/10/(9/10 + 9/17)], abs=TOL)

def test_OLMB1():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-1.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMB2():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-2.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMB3():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-3.response', matching="partial-corefud")
...
def test_OLMB4():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-4.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)
  assert evaluate(doc, mention_overlap) == (13/23, 1, 2*13/23/(1+13/23))

def test_OLMB5():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-5.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (3/4, 3/3, 6/7)
  assert evaluate(doc, b_cubed) == (17/24, 5/5, 34/41)
  assert evaluate(doc, mention_overlap) == (17/23, 1, 2*17/23/(1+17/23))

def test_OLMB6():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-6.response', matching="partial-corefud")
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMC1():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-1.response', matching="partial-corefud")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMC2():#the test case seems not correct
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-2.response', matching="partial-corefud")
...
def test_OLMC3():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-3.response', matching="partial-corefud")
  assert evaluate(doc, muc) == approx([3/4, 1, 6/7], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([17/24, 1, 34/41], abs=TOL)

def test_OLMC4():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-4.response', matching="partial-corefud")
  assert evaluate(doc, muc) == approx([1, 1, 1], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([1, 1, 1], abs=TOL)

@pytest.mark.skip(reason="errors in the test")
def test_OLMC5():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-5.response', matching="partial-corefud")
  assert evaluate(doc, muc) == approx([1, 1, 1], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([1, 1, 1], abs=TOL)
...
@pytest.mark.skip(reason="errors in the test")
def test_OLMC6():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-6.response', matching="partial-corefud")
...
@pytest.mark.skip(reason="errors in the test")
def test_OLMC7():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-7.response', matching="partial-corefud")
...
def test_HMA1():
  doc = read('TC-HMA.key', 'TC-HMA-1.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA2():
  doc = read('TC-HMA.key', 'TC-HMA-2.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA3():
  doc = read('TC-HMA.key', 'TC-HMA-3.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA4():
  doc = read('TC-HMA.key', 'TC-HMA-4.response', matching="head")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)

def test_HMA5():
  doc = read('TC-HMA.key', 'TC-HMA-5.response', matching="head")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)
