...
def read(key, response, matching):
...
def test_NCMA1():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-1.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA2():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-2.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA3():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-3.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA4():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-4.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA5():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-5.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMA6():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-6.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMA7():
  doc = read('noncontig_mentions/TC-NCMA.key', 'noncontig_mentions/TC-NCMA-7.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB1():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-1.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB2():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-2.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_NCMB3():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-3.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB4():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-4.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB5():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-5.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB6():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-6.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB7():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-7.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)

def test_NCMB8():
  doc = read('noncontig_mentions/TC-NCMB.key', 'noncontig_mentions/TC-NCMB-8.response', matching="partial")
  assert evaluate(doc, muc) == (2/3, 2/3, 2/3)
  assert evaluate(doc, b_cubed) == (7/10, 7/10, 7/10)
...
def test_OLMA1():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-1.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMA2():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-2.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1 + 9/23))

def test_OLMA3():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-3.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (18/23, 1, 2*18/23/(1+18/23))

def test_OLMA4():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-4.response', matching="partial")
...
def test_OLMA5():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-5.response', matching="partial")
...
def test_OLMA6():
  doc = read('overlapping_mentions/TC-OLMA.key', 'overlapping_mentions/TC-OLMA-6.response', matching="partial")
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMAA1():
  doc = read('overlapping_mentions/TC-OLMAA.key', 'overlapping_mentions/TC-OLMAA-1.response', matching="partial")
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMAA2():
  doc = read('overlapping_mentions/TC-OLMAA.key', 'overlapping_mentions/TC-OLMAA-2.response', matching="partial")
  assert evaluate(doc, mention_overlap) == approx([9/17, 9/10, 2*9/17*9/10/(9/10 + 9/17)], abs=TOL)

def test_OLMB1():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-1.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (1, 1, 1)

def test_OLMB2():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-2.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMB3():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-3.response', matching="partial")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == approx([3/4, 5/6, 15/19], abs=TOL)
  assert evaluate(doc, mention_overlap) == (13/23, 1, 2*13/23/(1+13/23))

def test_OLMB4():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-4.response', matching="partial")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)
  assert evaluate(doc, mention_overlap) == (13/23, 1, 2*13/23/(1+13/23))

def test_OLMB5():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-5.response', matching="partial")
  assert evaluate(doc, muc) == (3/4, 3/3, 6/7)
  assert evaluate(doc, b_cubed) == (17/24, 5/5, 34/41)
  assert evaluate(doc, mention_overlap) == (17/23, 1, 2*17/23/(1+17/23))

def test_OLMB6():
  doc = read('overlapping_mentions/TC-OLMB.key', 'overlapping_mentions/TC-OLMB-6.response', matching="partial")
  assert evaluate(doc, muc) == approx([1/2, 2/3, 4/7], abs=TOL)
  assert evaluate(doc, b_cubed) == (11/24, 7/10, 77/139)
  assert evaluate(doc, mention_overlap) == (9/23, 1, 2*9/23/(1+9/23))

def test_OLMC1():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-1.response', matching="partial")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_OLMC2():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-2.response', matching="partial")
  assert evaluate(doc, muc) == approx([3/4, 2/3, 12/17], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([3/4, 4/5, 24/31], abs=TOL)

def test_OLMC3():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-3.response', matching="partial")
  assert evaluate(doc, muc) == approx([3/4, 1, 6/7], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([17/24, 1, 34/41], abs=TOL)

def test_OLMC4():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-4.response', matching="partial")
  assert evaluate(doc, muc) == approx([1, 1, 1], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([1, 1, 1], abs=TOL)

def test_OLMC5():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-5.response', matching="partial")
  assert evaluate(doc, muc) == approx([1, 1, 1], abs=TOL)
  assert evaluate(doc, b_cubed) == approx([1, 1, 1], abs=TOL)
...
def test_OLMC6():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-6.response', matching="partial")
...
def test_OLMC7():
  doc = read('overlapping_mentions/TC-OLMC.key', 'overlapping_mentions/TC-OLMC-7.response', matching="partial")
...
def test_HMA1():
  doc = read('head_match/TC-HMA.key', 'head_match/TC-HMA-1.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA2():
  doc = read('head_match/TC-HMA.key', 'head_match/TC-HMA-2.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA3():
  doc = read('head_match/TC-HMA.key', 'head_match/TC-HMA-3.response', matching="head")
  assert evaluate(doc, muc) == (1, 1, 1)
  assert evaluate(doc, b_cubed) == (1, 1, 1)

def test_HMA4():
  doc = read('head_match/TC-HMA.key', 'head_match/TC-HMA-4.response', matching="head")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)

def test_HMA5():
  doc = read('head_match/TC-HMA.key', 'head_match/TC-HMA-5.response', matching="head")
  assert evaluate(doc, muc) == (3/4, 3/4, 3/4)
  assert evaluate(doc, b_cubed) == (17/24, 17/24, 17/24)
